...
export const config: RuntimeConfig = ${JSON.stringify(prunedConfig, null, 2)}

// Helper functions
export function getDependency(serviceName: string): ServiceEndpoint | undefined {
...
export const config = ${JSON.stringify(prunedConfig, null, 2)}

export function getDependency(serviceName) {
  return config.dependencies.find(dep => dep.service === serviceName)
}

export function getDependencyUrl(serviceName) {
  return getDependency(serviceName)?.url
}

export function getEnvironmentVar(key) {
  return config.environment[key]
}

export function getStaticEnvVar(key) {
  const envVar = config.environment[key]
  return envVar?.type === 'static' ? envVar.value : undefined
}
`
